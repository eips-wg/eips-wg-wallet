...
- `chainId`
  - **MUST** specify the integer ID of the chain as a hexadecimal string, per the [`eth_chainId`](https://eips.fyi/695) Ethereum RPC method.
...
For related work, see [EIP-3085: `wallet_addEthereumChain`](https://eips.fyi/3085) and [EIP-2015: `wallet_updateEthereumChain`](https://eips.fyi/2015).
...
- Display a confirmation whenever a `wallet_switchEthereumChain` is received, clearly identifying the requester and the chain that will be switched to.
  - The confirmation used in [EIP-1102](https://eips.fyi/1102) may serve as a point of reference.
